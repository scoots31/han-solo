...
These are the blocks Ren reads at every session open: always_loaded_core,
pending_thoughts, project_state, and portrait blocks.
"""
...
async def read_core_memory(block_label: str) -> str:
    ...
    """
    get_current_user()  # assert authenticated
...
async def write_core_memory(block_label: str, value: str) -> str:
    ...
    Returns confirmation with the updated block label.
    """
...
async def list_core_memory_blocks() -> list[dict]:
    """
    List all core memory blocks: label, character count, and first 100 chars of value.
    Useful for understanding what's currently in Ren's always-loaded context.
    """
    get_current_user()
